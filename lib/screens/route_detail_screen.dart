...
class RouteDetailScreen extends StatefulWidget {
  ...
  RouteDetailScreen({required this.routeNumber, required this.routeName});
...
class _RouteDetailScreenState extends State<RouteDetailScreen> {
  ...
  Future<void> _fetchBusStops() async {
    ...
    final url =
        '$baseUrl/get_bus_station_by_bus_number?bus_number=${widget.routeNumber}&direction=1';
    try {
  ...
  void _setMarkersAndPolyline(List<dynamic> stops) async {
    ...
    setState(() {
      ...
      if (_busStops.isNotEmpty) {
        _mapController
            .animateCamera(CameraUpdate.newLatLngZoom(_busStops.first, 14));
      }
    });
  ...
  void _onMapCreated(GoogleMapController controller) {
    ...
    // Tập trung và phóng to camera vào vị trí đầu tiên của tuyến
    if (_busStops.isNotEmpty) {
      _mapController.animateCamera(
        CameraUpdate.newLatLngZoom(_busStops.first, 14),
      );
    }
  }
